    ...
    <div className="w-full h-full flex flex-col bg-white">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider">Polls & Q&A</h3>
        </div>
        <div className="flex items-center gap-2">
          {isTeacher && (
            <button
              onClick={() => setShowCreate(!showCreate)}
              className="p-2 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-600 hover:bg-emerald-100 transition-all cursor-pointer"
            >
              <Plus size={14} />
            </button>
          )}
          ...
          >
            <div className="p-4 space-y-4 text-left">
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all font-semibold"
              />
                ...
                {options.map((opt, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all font-semibold"
                    />
                    {options.length > 2 && (
                      <button
              ...
              <div className="flex items-center gap-4 text-[10px]">
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    className="rounded text-emerald-500 border-slate-300"
                  />
                  <UserX size={12} className="text-slate-400" />
                  <span className="font-bold text-slate-500 uppercase tracking-wider">Anonymous</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    type="checkbox"
                  ...
                  <span className="font-bold text-slate-500 uppercase tracking-wider">Hide Votes</span>
                </label>
              </div>
              ...
              <div className="flex items-center gap-3">
                {options.length < 6 && (
                  <button
                    onClick={addOption}
                    className="text-[10px] font-bold text-emerald-600 hover:text-emerald-500 uppercase tracking-widest transition-colors border-none bg-transparent cursor-pointer"
                  >
                    + Add Option
                  </button>
                )}
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-bold text-slate-400 hover:bg-slate-100 transition-all border-none bg-transparent cursor-pointer"
                >
                  Cancel
                </button>
                <button
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar text-left">
        {polls.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center gap-3 py-12">
            ...
            <p className="text-xs font-bold text-slate-500">No active polls</p>
            {isTeacher && (
...
const PollCard: React.FC<{ poll: Poll; hideVotes: boolean }> = ({ poll, hideVotes }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm text-left"
    >
      <div className="flex items-start justify-between gap-3">
        <h4 className="text-sm font-black text-slate-800 leading-tight">{poll.question}</h4>
        <div className="flex items-center gap-1.5 shrink-0">
          {poll.anonymous && (
            <span className="px-2 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-slate-500 text-[8px] font-black tracking-widest uppercase">
              Anonymous
            </span>
          )}
        </div>
          ...
          return (
            <button
              ...
              className={`w-full relative overflow-hidden rounded-xl p-3 text-left transition-all border ${
                hasVoted
                  ? 'cursor-default'
                  : 'hover:bg-slate-100 cursor-pointer bg-white border-slate-200'
              } ${isSelected ? 'border-emerald-500 bg-emerald-50/20' : 'border-slate-150 bg-slate-50/50'}`}
            >
              {/* Progress bar */}
              {hasVoted && !hideVotes && (
                <motion.div
                  ...
                  className={`absolute inset-y-0 left-0 ${isSelected ? 'bg-emerald-500/10' : 'bg-slate-100'}`}
                />
              )}
              <div className="relative flex items-center justify-between gap-3 z-10">
                <div className="flex items-center gap-2 min-w-0">
                  {isSelected && <CheckCircle size={14} className="text-emerald-600 shrink-0" />}
                  <span className="text-xs font-bold text-slate-700 truncate">{opt.text}</span>
                </div>
                {hasVoted && !hideVotes && (
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[10px] font-bold text-slate-400">{opt.count}</span>
                    <span className="text-xs font-black text-slate-500 tabular-nums">{pct}%</span>
                  </div>
                )}
              </div>
            </button>
